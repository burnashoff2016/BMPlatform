...
1. Бэкенд: аутентификация отключена с помощью скрипта `disable_auth.py`
...
2. Если у вас есть отдельный сервер для бэкенда, убедитесь, что он запущен с отключенной аутентификацией:
   ```bash
   cd backend
...
2. Включите аутентификацию в бэкенде:
   ```bash
   python enable_auth.py
   ```
